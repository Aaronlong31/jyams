...
<div class="content">
	<div class="main_panel">
		...
		<div class="main_content">
			...
			<table class="table_normal" style="width: 1070px;margin:0">
				<tfoot>
					<tr>
						<th style="width:7%;text-align:center;">项目编号</th>
						<th style="width:22%;text-align:center;">项目名称</th>
						<th style="width:10%;text-align:center;">公司负责人</th>
						<th style="width:12%;text-align:center;">开工时间</th>
						<th style="width:12%;text-align:center;">完工时间</th>
						<th style="width:12%;text-align:center;">开票</th>
						<th style="width:12%;text-align:center;">收款</th>
					...
					<s:iterator value="buildingProjects">
						<tr>
							...
							<td style="text-align: center;" id="completion${projectId}">
								<s:if test="%{(loginUserId == project.companyPrincipalId)&&(status & 16) != 16}">
								...
								<s:elseif test="%{(status & 16) == 16}">
									...
									<s:property value="completionPersonName"/>
									<s:if test="%{loginUserId == project.companyPrincipalId}">
									<br><a href="javascript:void(0);" class="openA" onclick="change('open',${projectId})">重新开工</a>
									</s:if>
								</s:elseif>
							</td>
							<td style="text-align: center;" id="invoice${projectId}">
								<s:if test="%{(status & 32) == 32}">
...
$(function() {
	...
	function change(method, projectId){
		var url = "../buildingProject/" + method + "_ajax.action";
